...
import { Search, Plus, Trash2, Loader2, Edit, XCircle } from "lucide-react";
import { toast } from "sonner";
import Image from "next/image";
import { getProductsForBuyer } from "@/app/actions/products";
import { updateOrderAsBuyer, cancelOrderAsBuyer } from "@/app/actions/orders";
...
export default function BuyerOrderActions({
    ...
    if (status !== "PENDING_APPROVAL") {
        return null; // Actions only available for pending orders
    }
    ...
    }, [isEditOpen, initialItems, initialBuyerNote]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const res = await updateOrderAsBuyer(orderId, payload, buyerNote);

            if (res.success) {
                toast.success("message" in res ? res.message : "Pesanan berhasil direvisi!");
                setIsEditOpen(false);
            } else {
                toast.error("error" in res ? res.error : "Gagal merevisi pesanan.");
            }
        } catch (error) {
            toast.error("Terjadi kesalahan teknis saat merevisi pesanan.");
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleCancel = async () => {
        setIsCanceling(true);
        try {
            const res = await cancelOrderAsBuyer(orderId);
            if (res.success) {
                toast.success("message" in res ? res.message : "Pesanan berhasil dibatalkan!");
                setIsCancelOpen(false);
            } else {
                toast.error("error" in res ? res.error : "Gagal membatalkan pesanan.");
            }
        } catch (error) {
    ...
    return (
        <div className="flex gap-2">
            <Button size="sm" variant="outline" className="text-blue-600 border-blue-200 hover:bg-blue-50" onClick={() => setIsEditOpen(true)}>
                <Edit className="h-4 w-4 mr-1" /> Edit
            </Button>
            <Button size="sm" variant="outline" className="text-red-600 border-red-200 hover:bg-red-50" onClick={() => setIsCancelOpen(true)}>
            ...
            <Dialog open={isCancelOpen} onOpenChange={setIsCancelOpen}>
                <DialogContent>
                    ...
                    <div className="py-4">
                        <p>Anda yakin ingin membatalkan pesanan <strong>{orderId}</strong> secara permanen?</p>
                        <p className="text-sm text-neutral-500 mt-2">Pesanan akan dihapus dari sistem dan tidak dapat dikembalikan.</p>
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsCancelOpen(false)} disabled={isCanceling}>Tutup</Button>
            ...
            <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
                <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col p-0 shadow-2xl">
                    <DialogHeader className="p-6 border-b bg-neutral-50/50">
                        <DialogTitle className="text-xl font-bold">Revisi Pesanan Anda</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="flex-1 p-6 overflow-y-auto min-h-0 space-y-6">
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-xs text-neutral-500 font-mono">SKU: {product.sku} • {product.unit}</span>
                                                </div>
                                            </div>
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-semibold text-neutral-700">Catatan Khusus (Opsional)</label>
                            <Textarea 
                    ...
                    <DialogFooter className="p-6 border-t bg-neutral-50/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" className="font-bold h-11 px-6" onClick={() => setIsEditOpen(false)} disabled={isSaving}>
                                Tutup
                            </Button>
